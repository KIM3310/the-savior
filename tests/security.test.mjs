...
test("resolveCors ignores wildcard env entries and keeps an explicit trust boundary", () => {
  const env = { ALLOWED_ORIGINS: "*,https://allowed.example" };
  const options = { methods: "POST, OPTIONS", allowHeaders: "Content-Type" };
  ...
  assert.equal(allowed.allowed, true);
  assert.equal(allowed.headers["Access-Control-Allow-Origin"], "https://the-savior-9z8.pages.dev");
  assert.equal(blocked.allowed, false);
  assert.equal(blocked.headers["Access-Control-Allow-Origin"], undefined);
});
