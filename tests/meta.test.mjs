...
test("escalation readiness route exposes crisis guardrails", async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(body.contract_version, "the-savior-escalation-readiness-v1");
  assert.equal(body.summary.runtime_mode, "ollama-local");
  assert.equal(body.summary.fallback_visible, true);
  assert.ok(Array.isArray(body.guardrails));
  assert.equal(body.links.escalation_readiness, "/api/escalation-readiness");
});

test("progress trends route exposes public coaching deltas", async () => {
...
test("coach schema route exposes response contract", async () => {
  const response = await getCoachSchema(createContext("https://the-savior-9z8.pages.dev/api/schema/coach-response"));
  const body = await response.json();
