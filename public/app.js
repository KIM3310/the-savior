...
function renderFirstSessionGuide() {
  ...
  if (state.apiMisconfigured) {
    ...
    boundary = "키를 넣기 전에는 status page와 감정 초안만 차분히 준비하고, live AI는 나중에 여세요.";
...
function setupKeyboardShortcuts() {
  document.addEventListener("keydown", async (event) => {
    ...
    if (key === "r") {
      ...
      event.preventDefault();
      setRuntimeStatus("Shortcuts: Shift+R routes · Shift+P posture · Shift+C crisis · Shift+B status bundle · Shift+T breathing timer", "good");
    }
  });
}
...
function applyEscalationDecoration(reply, result) {
  const text = safeText(reply || "");
  if (!result || result.escalated !== true) return text;
  const resources = Array.isArray(result.resources) ? result.resources : [];
  const nextSteps = Array.isArray(result.next_steps) ? result.next_steps : [];
  const resourceLines = resources
    .map((item) => {
      const label = safeText(item && item.label ? item.label : "");
      const contact = safeText(item && item.contact ? item.contact : "");
      return label || contact ? `- ${label}${contact ? `: ${contact}` : ""}` : "";
    })
    .filter(Boolean);
  ...
  return [
    "[긴급 안내 · 사람 연결 우선]",
    text,
    resourceLines.length ? "" : null,
    resourceLines.length ? "즉시 연결 리소스" : null,
    ...resourceLines,
    nextStepLines.length ? "" : null,
    nextStepLines.length ? "지금 바로 할 일" : null,
    ...nextStepLines
  ]
...
function decorateCoachReply(reply, result) {
  if (result && result.escalated === true) {
    return applyEscalationDecoration(reply, result);
  }
  return applyFallbackDecoration(reply, result);
}

function applyRuntimeStatusFromResult(result, defaultSuccessMessage) {
  if (result && result.escalated === true) {
    setRuntimeStatus("긴급 지원 안내를 표시했습니다. 지금은 AI 코칭보다 즉시 사람 도움 연결이 우선입니다.", "error");
    return;
  }
...
function setupCheckinForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const reply = decorateCoachReply(result.reply || "결과가 비어 있습니다.", result);
...
function setupChatForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const reply = decorateCoachReply(result.reply || "응답이 비어 있습니다.", result);
