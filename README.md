...
## 하나의 대표 리뷰 시나리오
리뷰어는 잠들기 전 불안이 올라온 사용자가 1분 체크인을 남기고, 3분 안정 루틴과 코치 응답을 받은 뒤, live provider가 없어도 fallback으로 무너지지 않는 흐름을 바로 떠올릴 수 있어야 합니다. 그래서 이 저장소의 handoff는 감정 체크 → 응답 품질 → fallback 경계 순서로 첫 화면에서 읽히게 두었습니다.
...
- **프리세일즈 / 솔루션 세일즈 관점:** 사용자 가치와 운영 경계를 같은 데모 표면에서 설명할 수 있습니다.


## 포트폴리오 맥락
- **패밀리:** 사람 중심 / 웰니스 제품군
- **이 레포의 역할:** 감정 체크, 코치 응답, fallback 경계를 함께 보여주는 웰니스 운영 표면입니다.
- **연결해서 볼 레포:** `SteadyTap`, `ecotide`
